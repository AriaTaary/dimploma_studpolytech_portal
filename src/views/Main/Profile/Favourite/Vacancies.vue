    ...
    <div class="content content-profile">
      <div class="content-nav">
        <div class="content-title fav-title">
          <h1>Избранное:</h1>
          <router-link class="profile-settings-title not-active-title" :to="{ name: 'PersonalFavouriteArticles', params: { username: this.$store.getters.getUser.username } }">Статьи</router-link>
          <router-link class="profile-settings-title" :to="{ name: 'PersonalFavouriteVacancies', params: { username: this.$store.getters.getUser.username } }">Вакансии</router-link>
        </div>
      </div>
